    ...
    try {
      ...
      if (location.lat && location.lng && location.lat !== 0 && location.lng !== 0) {
        try {
          ...
          // Fail-safe: if duplicate check fails, proceed with submission
          console.warn('[Automation] Duplicate check failed, proceeding with submission');
        }
      }
        ...
        history: [
          {
            ...
            note: language === 'en' ? 'Complaint registered' : 'പരാതി രജിസ്റ്റർ ചെയ്തു',
          },
        ],
      };
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1 container py-8">
        <div className="max-w-2xl mx-auto">
          ...
          {/* Feature 4: Duplicate Detection Dialog */}
          {showDuplicateDialog && duplicateInfo && (
            <div className="card-elevated p-6 mb-6 border-2 border-amber-400 dark:border-amber-600 animate-slide-up">
              ...
              <div className="bg-secondary/50 rounded-lg p-4 mb-4">
                <p className="text-sm font-mono text-muted-foreground">{duplicateInfo.duplicateTrackingId}</p>
                <p className="font-medium text-foreground mt-1">{duplicateInfo.duplicateTitle}</p>
                <div className="flex gap-4 mt-2 text-xs text-muted-foreground">
                  <span>📍 {Math.round(duplicateInfo.distanceMeters)}m away</span>
                  <span>👥 {duplicateInfo.currentSupportCount + 1} supporters</span>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                <Button
                  ...
                  onClick={async () => {
                    await api.incrementSupport(duplicateInfo.duplicateId);
                    toast.success(
                      language === 'en'
                        ? 'Your support has been registered!'
                        : 'നിങ്ങളുടെ പിന്തുണ രജിസ്റ്റർ ചെയ്തു!'
                    );
                    setShowDuplicateDialog(false);
                    setDuplicateInfo(null);
                    navigate(`/track?id=${duplicateInfo.duplicateTrackingId}`);
                  }}
                >
                  <span className={language === 'ml' ? 'font-malayalam' : ''}>
                    {t.supportExisting}
                ...
                <Button
                  ...
                  className="flex-1"
                  onClick={() => {
                    // Allow submission despite duplicate — dialog is already shown
                    setShowDuplicateDialog(false);
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            {/* Fields below are only shown after a sector is selected */}
            {selectedSector && (
